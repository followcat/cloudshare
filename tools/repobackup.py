...
if __name__ == '__main__':
    ...
    for folder in backup_folders:
        ...
        SVC_MSG.backup(message_backup_path)
        SVC_PWD.backup(password_backup_path)
        SVC_ACCOUNT.backup(account_backup_path)
        SVC_CO_REPO.backup(corepo_backup_path)
        SVC_CV_REPO.backup(cvrepo_backup_path)
        SVC_JD_REPO.backup(jdrepo_backup_path)
        SVC_PEO_REPO.backup(peorepo_backup_path)
        SVC_CV_INDIV.backup(cvindiv_backup_path)
        SVC_PEO_INDIV.backup(peoindiv_backup_path)
