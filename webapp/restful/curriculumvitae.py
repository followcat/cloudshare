...
class CurrivulumvitaeAPI(Resource):
    ...
    def get(self, id):
        md = self.svc_cv.getmd(id)
        yaml = self.svc_cv.getyaml(id)
...
class CurrivulumvitaeMDAPI(CurrivulumvitaeAPI):
    ...
    def put(self, id):
        ...
        self.svc_cv.modify(name.md, md_data.encode('utf-8'), committer=user.id)
        return { 'result': True }
...
class CurrivulumvitaeYAMLAPI(CurrivulumvitaeAPI):
    ...
    def get(self, id):
        yamlinfo = self.svc_cv.getyaml(id)
    ...
    def put(self, id):
        ...
        name = core.outputstorage.ConvertName(id)
        yamlinfo = self.svc_cv.getyaml(filename)
        ...
        for key, value in updateinfo.iteritems():
            ...
            self.svc_cv.modify(name.yaml, yaml.safe_dump(yamlinfo, allow_unicode=True),
