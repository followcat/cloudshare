...
class LSIsimilarity(object):
    ...
    def probability(self, doc, top=None):
        """
            ...
            (100, 50, 25, 0)
            >>> assert kgr_percentage('9bbc45a81e4511e6b7066c3be51cefca', jd_service, sim, percentage=int(float(2)/3*100))
            ...
            >>> assert kgr_bad('7cadbda40b5d11e699956c3be51cefca', jd_service, sim)
            ...
            >>> assert kgr_bad('763199560a9411e6a7936c3be51cefca', jd_service, sim)
        ...
        FKJY new:
            ...
            >>> assert kgr_percentage('78df9d86555f11e6abad4ccc6a30cd76', jd_service, sim, cvs=['5ziy6c80', '6y0a1a75', '80clrjqi', 'rlb3jau0', 'sbip7deq', 'uwbvmsod', '68oojytn'], percentage=int(float(1)/7*100)) #ranks {'68oojytn': 84, 'rlb3jau0': 22, '5ziy6c80': 176, '80clrjqi': 5, '6y0a1a75': 69, 'sbip7deq': 576, 'uwbvmsod': 151}
        ...
        IBA new:
            ...
            >>> assert kgr_percentage('86119050313711e69b804ccc6a30cd76', jd_service, sim, cvs=['dpaxyqns', 'kf9sxzox', '5o4tiazc', 'n2ae2kyt', 'jc496tc2', 'hieheubl', 'rzcqg8m3'], percentage=int(float(1)/7*100))
            >>> assert kgr_percentage('9b48f97653e811e6af534ccc6a30cd76', jd_service, sim, cvs=['6r03u6so', '8fq1dwq3', 'dg2n5hqa'], percentage=int(float(2)/3*100))
